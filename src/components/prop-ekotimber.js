...
const EkotTimberProp = () => {
    ...
    const handleNext = () => {
      ...
      return (
        <div className="lg:max-w-[1500px] max-w-[500] mx-auto pt-4 mt-20  bg-gradient-to-r from-green-700 bg-accent border rounded-[50px]">
            ...
            {imagesToShow.map((property, index) => (
              <div key={index} className="w-[20%] p-2 hover:shadow-lg transition duration-300">
                <div className="border rounded-lg p-4">
                  <img
                    className="rounded-lg bg-white max-h-[200px] lg:max-h-[600px] lg:w-full mb-2 transform transition-transform hover:scale-105"
                    src={property.image.type}
                    alt=""
                  />
                  <div className="text-center lg:text-xl text-sm font-medium text-white capitalize">{property.title}</div>
                </div>
              </div>
            ))}
          </div>
